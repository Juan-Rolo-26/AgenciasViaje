...
export default function DestinoDetail() {
  ...
  const backLink =
    isArgentinaDestination(destino.paisRegion) ? "/argentina" : "/destinos";
        ...
        <section className="destination-packages-premium" id="paquetes">
          {!selectedPackage ? (
            <div className="packages-container">
              {(() => {
                const ofertasPaquetes = ofertasDestino.filter(o => !SALIDAS_GRUPALES_SLUGS.has(o.slug));

                return (
                  <>
                    {/* SECTION: PAQUETES (INDIVIDUALES) */}
                    {ofertasPaquetes.length > 0 && (
                      <div className="packages-section-individual" style={{ marginTop: '0' }}>
